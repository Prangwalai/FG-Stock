...
<div class="container mt-4">
  ...
  <h2 class="mb-0" style="text-align: center; color: blue;">รายงานยอดสต๊อกน้ำถัง 18.9 ลิตร คงเหลืองผลิตประจำวัน : วังน้อย</h2>
  </div>
    ...
    <tbody>
      <?php foreach($summary as $item): ?>
        <tr>
          ...
          <td><button type="button" class="btn btn-light btn-sm" onclick="window.location.href='<?= base_url("wfg/monthlyDetail/{$item->year}/{$item->month}") ?>'">
  ข้อมูลรายงาน
</button></td>
          <td><?= date('d/m/Y H:i น.', strtotime($item->Date)) ?></td>
          <td><a class="btn btn-primary btn-sm" href="<?= base_url("wfg/monthlyDetail/{$item->year}/{$item->month}") ?>">ข้อมูล</a>
          <a href="<?= base_url("wfg/exportPDF/{$item->year}/{$item->month}") ?>" class="btn btn-danger btn-sm" target="_blank">Export PDF</a></td>
        </tr>
      <?php endforeach; ?>
    </tbody>
      </tbody>
