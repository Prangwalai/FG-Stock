...
<div class="container mt-5">
    <h2 class="text-center mb-4 text-primary">รายการบันทึกข้อมูลประจำวัน : ดอนเมือง</h2> 
    
    <form method="get" action="<?= base_url('fg/dailyView') ?>" class="row g-3 mb-4 align-items-end">
    <div class="col-auto">
    ...
    <div class="col-auto d-flex gap-2">
        ...
        <button type="button" class="btn btn-sm btn-secondary"
            onclick="window.location.href='<?= site_url('fg/dmhome') ?>'">
            ...
            <table class="table table-bordered table-striped align-middle" id="stockTable">
                ...
                <tbody>
                    <?php foreach ($records as $record): ?>
                        <tr>
                            <td class="text-center"><?= esc($record['Date']) ?></td>
                            <td class="text-center">
                                <a href="<?= site_url('/fg/dmview/' . $record['Date']) ?>" class="btn btn-sm btn-light me-2">
                                    <i class="fas fa-eye"></i> ดูรายละเอียด
                                </a>
                                <a href="<?= site_url('/fg/dmedit/' . $record['Date']) ?>" class="btn btn-sm btn-warning">
...
<script>
  ...
  // ตั้งค่าเมื่อคลิกปุ่มยืนยัน
    ...
    window.location.href = '<?= site_url('/fg/delete/') ?>' + record.ID;
    };
  }
