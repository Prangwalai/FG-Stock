...
<head>
    ...
    <title>บันทึกรายงานสต๊อกคลังสำเร็จรูป : ดอนเมือง</title>
...
<div class="container mt-4">
    <form  method="post" action="<?= site_url('/fg/create') ?>">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="mb-0">บันทึกรายงานสต๊อกคลังสำเร็จรูป : ดอนเมือง</h1>
        <div class="d-flex align-items-center">
            <label for="record_date" class="form-label me-4 mb-2">Date:</label>
            <input type="date" id="record_date" name="record_date" class="form-control" value="<?= date('Y-m-d') ?>">
        </div>
    </div>
        <?php 
        ...
        <div class="card mb-3">
            ...
            <div class="card-body row">
                <?php 
                foreach($items as $it): 
                    if ($it->section === $sec->section_code): 
                ?>
                    <div class="col-md-6 mb-3">
                       <label class="form-label"><?= $it->item_name ?></label>
                       <div class="input-group">
                         <input type="number" name="item[<?= $it->item_code ?>]" class="form-control" placeholder="Enter value" value="0"
        ...
        <button type="button"  class="btn btn-dark" onclick="window.location.href='<?= base_url('/dm') ?>'">ย้อนกลับ</button>
        </div>
...
<!-- Bootstrap JS (optional if you use modal or components that require JS) -->
